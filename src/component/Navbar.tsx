...
const Navbar: React.FC = () => {
  ...
  const linkClass = ({ isActive }: { isActive: boolean }) =>
    ...
    ${isActive ? "after:w-full text-white" : "after:w-0 hover:after:w-full text-zinc-300 hover:text-red-600"}`;

  const mobileLinkClass = ({ isActive }: { isActive: boolean }) =>
    `block text-center font-bold text-sm uppercase tracking-widest py-3 border-b border-zinc-800 transition
    ${isActive ? "text-red-500" : "text-zinc-300 hover:text-red-500"}`;
    ...
    <nav className="fixed top-0 left-0 w-full bg-black text-white h-20 border-b border-zinc-800 z-50">
      <div className="max-w-7xl mx-auto h-full px-6 flex items-center justify-between relative">
        ...
        <div className="flex lg:hidden items-center gap-4 text-xl">
          <button onClick={() => setMenuOpen(!menuOpen)} className="text-zinc-300 hover:text-red-600 transition">
            {menuOpen ? <FiX /> : <FiMenu />}
          </button>
        </div>
        ...
        <div className="hidden lg:flex items-center gap-8 w-[40%] justify-end">
          <NavLink to="/coffee" className={linkClass}>Coffee</NavLink>
          <NavLink to="/subscribe" className={linkClass}>Subscribe</NavLink>
          <NavLink to="/merch" className={linkClass}>Merch</NavLink>
        </div>

        {/* Logo */}
        <div className="absolute left-1/2 -translate-x-1/2 top-3 z-[100]">
          <NavLink to="/">
            <div className="w-24 h-24 lg:w-28 lg:h-28 rounded-full border-4 border-red-600 overflow-hidden bg-black flex items-center justify-center">
        ...
        <div className="hidden lg:flex items-center justify-between w-[40%]">
          ...
          <div className="flex items-center gap-5 text-xl">
            <NavLink to="/auth" className="text-zinc-300 hover:text-red-600 transition"><FiUser /></NavLink>
            <button className="text-zinc-300 hover:text-red-600 transition"><FiSearch /></button>
      ...
      {menuOpen && (
        <div className="lg:hidden bg-black border-t border-zinc-800 px-6 pb-4">
          {["/coffee", "/subscribe", "/merch", "/store", "/blog", "/about"].map((path) => (
            <NavLink key={path} to={path} className={mobileLinkClass} onClick={() => setMenuOpen(false)}>
              {path.replace("/", "")}
            </NavLink>
          ))}
        </div>
